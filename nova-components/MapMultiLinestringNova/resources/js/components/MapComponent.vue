...
export default {
    ...
    methods: {
        initMap() {
            setTimeout(() => {
                const center = (this.field.center) ? this.field.center : (this.center) ? this.center : DEFAULT_CENTER;
                const defaultZoom = (this.field.defaultZoom) ? this.field.defaultZoom : DEFAULT_DEFAULTZOOM;
                const linestringGeojson = this.field.geojson;
                mapDiv = L.map(this.mapRef).setView(center, defaultZoom);

                L.tileLayer(
                    (this.field.tiles) ? this.field.tiles : DEFAULT_TILES,
                    {
                        attribution: (this.field.attribution) ? this.field.attribution : DEFAULT_ATTRIBUTION,
