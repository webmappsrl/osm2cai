...
class DumpDb extends Command {
    ...
    public function handle() {
        ...
        try {
            ...
            Log::info('osm2cai:dump_db -> Database dump created successfully in ' . $dumpFileName);
            ...
            Log::error('osm2cai:dump_db -> The dump process cannot be initialized: ' . $e->getMessage());
            Log::error('osm2cai:dump_db -> Make sure to clear the config cache when changing the environment: `php artisan config:cache`');

            return 2;
        } catch (DumpFailed $e) {
            Log::error('osm2cai:dump_db -> Error while creating the database dump: ' . $e->getMessage());
